"""Subprocess entry point for the Stroop color-word interference-control
game (see app/tasks/attention_focus.py for the full rationale, and
...
(demo-scaled) baseline duration and the trial's shuffled word/ink-color
stimulus list into a bootstrap JSON file passed via --bootstrap. This
process only renders one trial's baseline + stimulus stream, drives its own
event loop, and logs -- it never touches randomness itself.

Usage (normally only invoked by attention_focus.py, not run directly):
    python -m app.webui.stroop_app --session-dir sessions/P001_123 \\
...
FRONTEND_DIR = Path(__file__).resolve().parent / "stroop"


class StroopApi(WebTaskApi):
...
def main() -> int:
    ...
    task_label = "familiarization" if args.practice else "attention_focus_stroop"
    event_logger = EventLogger(Path(args.session_dir), args.session_id, args.participant_id)
    api = StroopApi(event_logger, task_label, bootstrap)
